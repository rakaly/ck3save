...
pub fn text_header_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("owned", |b| {
        b.iter(|| Ck3Extractor::builder().extract_header_owned(HEADER_TXT).unwrap());
    });
    group.bench_function("borrowed", |b| {
        b.iter(|| Ck3Extractor::builder().extract_header_borrowed(HEADER_TXT).unwrap());
    });
    group.finish();
}
...
pub fn binary_header_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("owned", |b| {
        b.iter(|| Ck3Extractor::builder().extract_header_owned(HEADER_BIN).unwrap());
    });
    group.bench_function("borrowed", |b| {
        b.iter(|| Ck3Extractor::builder().extract_header_borrowed(HEADER_BIN).unwrap());
    });
    group.finish();
}
