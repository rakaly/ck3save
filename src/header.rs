...
impl SaveHeader {
    pub fn from_slice(data: &[u8]) -> Result<Self, Ck3Error> {
        ...
        let kind_hex = std::str::from_utf8(&data[5..7]).map_err(|_| Ck3ErrorKind::InvalidHeader)?;
        let kind = u16::from_str_radix(kind_hex, 16).map_err(|_| Ck3ErrorKind::InvalidHeader)?;
        let random = take::<8>(&data[7..15]);

        let meta_hex =
            std::str::from_utf8(&data[15..23]).map_err(|_| Ck3ErrorKind::InvalidHeader)?;
        let meta_len =
            u64::from_str_radix(meta_hex, 16).map_err(|_| Ck3ErrorKind::InvalidHeader)?;
